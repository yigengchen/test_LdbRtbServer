...
void CMd5::Md5Final(UCHAR digest[16])
{
	UINT uiCount = 0;;
	UCHAR *pcCur = NULL;
	...
	*pcCur ++ = 0x80;
	...
	if( uiCount < 8 ) {
		...
		Md5ByteReverse(m_ucIn, 16);
		Md5Transform(m_uiBuf, (UINT*)m_ucIn);
	...
	Md5ByteReverse(m_ucIn, 14);

	((UINT*)m_ucIn)[14] = m_uiBits[0];
	((UINT*)m_ucIn)[15] = m_uiBits[1];

	Md5Transform(m_uiBuf, (UINT*)m_ucIn);
	Md5ByteReverse((UCHAR*)m_uiBuf, 4);
	memcpy(digest, m_uiBuf, 16);

	return;
}

void CMd5::Md5Update(UCHAR* puszBuf, UINT uiLen)
{
	UINT uiValue = 0;
	UCHAR *pcCur = NULL;
	uiValue = m_uiBits[0];
...
void CMd5::Md5Transform(UINT buf[4], UINT in[16])
{
...
void CMd5::Md5ByteReverse(UCHAR* puszBuf, UINT uiLongs)
{
	UINT uiValue;
	do {
...
unsigned char * CMd5::SHA1_Encode( const char *src)
{
        SHA_CTX c;
